...
- Settings → AI coach has a kill switch: anyone who can open the dashboard can turn the coach OFF (it stops spending at once); turning it back ON requires `ADMIN_PASSWORD`. The same page shows a usage/cost table from an append-only ledger of every model call.
